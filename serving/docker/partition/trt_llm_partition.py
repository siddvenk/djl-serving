...
def build_engine(sanitized_properties: dict, model_id: str,
                 output_save_dir: str, tensor_parallel_degree: int,
                 pipeline_parallel_degree: int):
    trtllm_args, llm_kwargs = parse_properties_for_trtllm(sanitized_properties)
    ...
    logger.info(
        f"[LMI] Model Compiled successfully, saving to {output_save_dir}")
    llm_model.save(output_save_dir)
    # TODO - add serving.properties for converted model
...
def parse_properties_for_trtllm(properties: dict) -> Tuple[List[str], dict]:
    llm_kwargs = {}
