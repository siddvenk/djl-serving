...
def parse_quant_config(properties: dict) -> Optional[QuantConfig]:
    ...
    if "quant_algo" in properties:
        quant_config["quant_algo"] = QuantAlgo(properties.pop("quant_algo").upper())
    if "kv_cache_quant_algo" in properties:
        quant_config["kv_cache_quant_algo"] = QuantAlgo(properties.pop(
...
def parse_llm_kwargs(properties: dict) -> dict:
    ...
    if "backend" in properties:
        llm_kwargs["backend"] = properties.pop("backend")
    return llm_kwargs
