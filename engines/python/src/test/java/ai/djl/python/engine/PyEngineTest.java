...
public class PyEngineTest {
    ...
    @Test
    public void testContinuousBatch() throws TranslateException, IOException, ModelException {
        int inferenceRequestSleepTime = 5;
        Criteria<Input, Output> criteria =
                Criteria.builder()
                        .setTypes(Input.class, Output.class)
                        .optEngine("Python")
                        .optModelPath(Paths.get("src/test/resources/continuous_batch"))
                        .build();
            ...
            for (int i = 0; i < 5; ++i) {
                ...
                input.add(
                        "{\"inputs\": \"request"
                                ...
                                + "}}");
                input.addProperty("Content-Type", "application/json");
                input.addProperty("requestId", "testRequestId" + i);
                Output output = predictor.predict(input);
