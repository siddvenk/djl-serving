...
async def wrapped_vllm_stream_generator(
        generator: AsyncGenerator,
        properties: dict) -> AsyncGenerator[Output, None]:
    async for chunk in generator:
        # TODO: there is probably a more efficient approach than creating Outputs for each
        ...
        # TODO: We need to transform the LMI spec to correct form here
        chunk_response = {
            "data": trimmed_chunk,
            "last": last,
        }
...
class VLLMHandler:
    ...
    def preprocess_request(
        ...
        raw_request = batch[0]
        client_request_id = raw_request.get_property("requestId")
        assert client_request_id is not None, "requestId cannot be None"
    ...
    async def inference(
        ...
        try:
            ...
            logging.error("Input parsing failed")
            output = Output()
        ...
        if request.stream:
            logging.info("Streaming request, adding async generator to output")
            return wrapped_vllm_stream_generator(response, properties)
